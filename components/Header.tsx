...
const Header: React.FC<HeaderProps> = ({ 
  ...
  // Official Logo Assets - Single SVG for maximum fidelity and correct alignment
  const LexiBranding = () => (
    <div className="flex items-center gap-2 h-8 sm:h-9">
      <svg 
        id="LexiLift_Full_Logo" 
        xmlns="http://www.w3.org/2000/svg" 
        viewBox="0 0 671.29 169.23" 
      ...
      {isPro && (
        <span className="flex-shrink-0 mb-auto text-[8px] font-black text-white bg-gradient-to-r from-cyan-400 to-indigo-500 px-1.5 py-0.5 rounded shadow-sm">
          <Crown size={7} fill="currentColor" className="inline mr-0.5" /> {t.proLabel}
        </span>
      )}
    </div>
  );
    ...
    <header className="bg-white/90 backdrop-blur-md border-b border-slate-200 px-3 sm:px-4 py-2 sm:py-2.5 z-50 flex items-center justify-between shadow-sm shrink-0 sticky top-0 safe-top">
      ...
      <div className={`flex items-center gap-1.5 sm:gap-3 ${isRtl ? 'flex-row-reverse' : 'flex-row'}`} dir={isRtl ? 'rtl' : 'ltr'}>
        <div className="relative group">
           <div className={`flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-full border shadow-sm transition-all ${isPro ? 'bg-cyan-50 border-cyan-100 text-indigo-700' : 'bg-blue-50 border-blue-100 text-blue-600'}`}>
              <Zap size={12} fill="currentColor" className={isPro ? 'text-cyan-500' : 'text-blue-600'} />
              <span className="text-[11px] sm:text-sm font-black">{sparks}</span>
           </div>
           
           <button className="absolute -top-2 -right-1 bg-white border border-slate-200 rounded-full p-0.5 text-slate-400 hover:text-cyan-600 hover:border-cyan-200 shadow-sm transition-all active:scale-90">
